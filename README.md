...
可以看到我们需要的图片文件在data/data55873/images文件夹下，train、test各200张，val为100张。
...
class BSD_data(Dataset):
    ...
    """
    def __init__(self, mode='train',image_path="data/data55873/images/"):
        """
...
```
    ...
    <paddle.fluid.core_avx.CPUPlace at 0x7fe758969ab0>
...
```
    ...
    step 13/13 [==============================] - loss: 0.0151 - 2s/step        
...
```python
# 加载保存的模型
model.load('checkpoint/model_final')

predict_results = model.predict(val_dataset)
```

    Predict begin...
    step 100/100 [==============================] - 47ms/step         
    Predict samples: 100
...
```python
main(model,'data/data55873/images/test/15011.jpg')
```
...
![png](output_28_4.png)
    ...
    psmr_low: 27.0987208212598 psmr_pre: 25.960705981345733
...
本项尝试使用paddle2.0来完成全流程开发。演示了数据集的定义，模型组网，模型训练等关键步骤。

总体感觉paddle2.0的api设计更加合理，融合了pytorch、TF、keras等框架的优点，上手难度教之前版本更低。

# 9. 自我介绍

金陵科技学院 国际教育学院 软件工程（中澳合作办学） 2017级 陆俊陶

邮箱：juntao.lu@connect.qut.edu.au（欢迎骚扰，一起探讨学习）

目前就读于 昆士兰科技大学（QUT） ps：曾经火爆全球的手游《水果忍者》就是这个学校的作品

近期目标：拿到墨大AI方向研究生的offer（好像有点难度）

爱好倒腾，喜欢航模、航拍，梦想有朝一日实现财富自由，带着我的小飞机自驾拍遍全国

疫情原因暂时还在国内，欢迎南京的小伙伴找我面基。
